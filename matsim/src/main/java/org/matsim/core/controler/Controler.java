...
public class Controler extends AbstractController {
    ...
    // * Seems to me that we should try to get everything here final.
    // Flexibility is provided by the ability to set or add factories. If this
    // is
    ...
    // DefaultControlerModule includes submodules. If you want less than what
    // the Controler does
    // by default, you can leave ControlerDefaultsModule out, look at what it
    // does,
    // and only include what you want.
    private List<AbstractModule> modules = Arrays
	    .<AbstractModule> asList(new ControlerDefaultsModule());

    // The module which is currently defined by the sum of the setXX methods
    // called on this Controler.
    ...
    private Controler(final String configFileName, final Config config,
	...
	if (scenario != null) {
	    ...
	    this.config
		    .addConfigConsistencyChecker(new ConfigConsistencyCheckerImpl());
	} else {
	...
	this.snapshotWriterRegister = snapshotWriterRegistrar
		.getFactoryRegister();
	...
	synchronized (this) {
	    setupOutputDirectory(this.config.controler().getOutputDirectory(),
		    this.config.controler().getRunId(), this.overwriteFiles);
	}
	if (this.config.scenario().isUseTransit()) {
	    setupTransitSimulation();
	}
	loadData();
	// I find it difficult (to teach) that the behavior between the
	// constructor and controler.run() depends on which constructor
	// you call. kai, apr'14
	...
	// checkConfigConsistencyAndWriteToLog(config,
	// "config dump before iterations start" ) ;
	// prepareForSim();
	// doIterations(config.controler().getFirstIteration(),
	...
	ActivityParams transitActivityParams = new ActivityParams(
		PtConstants.TRANSIT_ACTIVITY_TYPE);
	transitActivityParams.setTypicalDuration(120.0);

	// The following two lines were introduced in nov/12. _In addition_, the
	// conversion of ActivityParams to
	// ActivityUtilityParameters will set the scoreAtAll flag to false (also
	// introduced in nov/12). kai, nov'12
	...
	// No, I think that the user-defined parameters are set later, in fact
	// overwriting this setting here.
	// kai, nov'12
	...
	if (this.dumpDataAtEnd) {
	    this.addCoreControlerListener(new DumpDataAtEnd(scenarioData,
		    getControlerIO()));
	}

	if (this.getScoringFunctionFactory() == null) {
	    this.setScoringFunctionFactory(ControlerDefaults
		    .createDefaultScoringFunctionFactory(this.scenarioData));
	}

	this.injector = Injector.createInjector(config, new AbstractModule() {
	    @Override
	    public void install() {
		// Use all the modules set with setModules, but overriding them
		// with things set with
	...
	this.injector.retrofitScoringFunctionFactory(this
		.getScoringFunctionFactory());
	PlansScoring plansScoring = new PlansScoring(this.scenarioData,
		this.events, getControlerIO(), this.getScoringFunctionFactory());
	this.addCoreControlerListener(plansScoring);

	this.addCoreControlerListener(new PlansReplanning(this.injector
		.getInstance(StrategyManager.class), getScenario()
		.getPopulation()));
	this.addCoreControlerListener(new PlansDumping(this.scenarioData, this
		.getConfig().controler().getFirstIteration(), this.config
		.controler().getWritePlansInterval(), this.stopwatch, this
		.getControlerIO()));

	this.addCoreControlerListener(new EventsHandling(this.events, this
		.getConfig().controler().getWriteEventsInterval(), this
		.getConfig().controler().getEventsFileFormats(), this
		.getControlerIO()));
	// must be last being added (=first being executed)

	Set<EventHandler> eventHandlersDeclaredByModules = this.injector
		.getEventHandlersDeclaredByModules();
	for (EventHandler eventHandler : eventHandlersDeclaredByModules) {
	    this.events.addHandler(eventHandler);
	}
	Set<ControlerListener> controlerListenersDeclaredByModules = this.injector
		.getControlerListenersDeclaredByModules();
    ...
    /**
     * Empty hook for subclasses to load more ControlerListeners. Please do not
     * use. Subclassing Controler is discouraged.
     */
    @Deprecated
     ...
     * scenario data (network, population) is read, and after all
     * ControlerListeners have processed their startup event.
     * <p/>
     * Design comments/questions:
     * <ul>
     * <li>"from the configuration" sounds too narrow. Should be something like
     * "from everything that is there at this point, including, say, factories."
     * kai, dec'12
     * </ul>
     */
    @Deprecated
    // overwriting this method is deprecated. Please talk to MZ or KN if you
    // think that you really need this. nov'14
	...
	// yyyy cannot make this final: overridden at about 15 locations. kai,
	// jan'13
	...
	// overridden once for a test case (not so bad since it is package
	// protected). kai, jan'13
	if (this.thisMobsimFactory != null) {
	    Mobsim simulation = this.thisMobsimFactory.createMobsim(
		...
		&& ((SimulationConfigGroup) this.config
			.getModule(SimulationConfigGroup.GROUP_NAME))
			.getExternalExe() != null) {
	    ExternalMobsim simulation = new ExternalMobsim(this.scenarioData,
		    this.events);
	    simulation.setControlerIO(this.getControlerIO());
	    simulation.setIterationNumber(this.getIterationNumber());
	    return simulation;
	} else {
	    MobsimFactory f = this.mobsimFactoryRegister
		    .getInstance(this.config.controler().getMobsim());
	    Mobsim simulation = f.createMobsim(this.getScenario(),
		    this.getEvents());
	    enrichSimulation(simulation);
	    return simulation;
	}
	...
	if (simulation instanceof ObservableMobsim) {
	    ...
	    if (config.controler().getWriteSnapshotsInterval() != 0
		    && this.getIterationNumber()
			    % config.controler().getWriteSnapshotsInterval() == 0) {
		SnapshotWriterManager manager = new SnapshotWriterManager(
			config);
		for (String snapshotFormat : this.config.controler()
			.getSnapshotFormat()) {
		    SnapshotWriterFactory snapshotWriterFactory = this.snapshotWriterRegister
			    .getInstance(snapshotFormat);
		    String baseFileName = snapshotWriterFactory
			    .getPreferredBaseFilename();
		    String fileName = this.getControlerIO()
			    .getIterationFilename(this.getIterationNumber(),
				    baseFileName);
		    SnapshotWriter snapshotWriter = snapshotWriterFactory
			    .createSnapshotWriter(fileName, this.scenarioData);
		    manager.addSnapshotWriter(snapshotWriter);
		}
		((ObservableMobsim) simulation)
			.addQueueSimulationListeners(manager);
	    }
	}
    ...
    // meaning mostly from ControlerListeners, except possibly the
    // StartupListener,
    // as the TravelTimeCalculator and the TripRouterFactory aren't available
    // there yet.
    //
    ...
    // and then change something on it. Or wrap it in something else and set it
    // again.
     ...
     * This is a routing service which you can use to calculate routes, e.g.
     * from your own replanning code or your own within-day replanning agent
     * code. You get a Provider (and not an instance directly) because your code
     * may want to later create more than one instance. A TripRouter is not
     * guaranteed to be thread-safe, so you must get() an instance for each
     * thread if you plan to write multi-threaded code.
     * 
     * See {@link org.matsim.core.router.TripRouter} for more information and
	...
	throw new RuntimeException(
		"To modify scoring for your Agents, please either:"
    ...
    // ******** --------- *******
    // The following methods are the outer interface of the Controler. They are
    // used
    // to set up infrastructure from the outside, before calling run().
    // Some of them may also work from the StartupListeners, I haven't sorted
    // that out yet.
    // Contrast to the outermost interface, see below.
    // ******** --------- *******

    /**
     * It should be possible to add or remove MobsimListeners between
     * iterations, no problem.
     */
	...
	this.addOverridingModule(new AbstractModule() {
	    @Override
	    public void install() {
		bindToInstance(TravelDisutilityFactory.class,
    ...
    public final void setTerminationCriterion(
    ...
    /**
     * Allows you to set a factory for {@link org.matsim.core.router.TripRouter}
     * instances. Do this if your use-case requires custom routing logic, for
     * instance if you implement your own complex travel mode. See
     * {@link org.matsim.core.router.TripRouter} for more information and
     * pointers to examples.
     */
	...
	this.overrides = AbstractModule.override(Arrays.asList(this.overrides),
		abstractModule);
    }
     ...
     *            not (false)
     * @deprecated the danger represented by this method seems too great
     *             compared to the small convenience it brings: it will thus be
     *             removed soon. If you are sure that you want to get rid of
     *             previous results, call
     *             {@link org.matsim.core.utils.io.IOUtils#deleteDirectory(java.io.File)}
     *             on your output directory. If you want to write stuff to the
     *             output directory before lanching the controler, you might (i)
     *             write this stuff to another directory, or (ii) put your IO
     *             code in a StartupListenner, to get it executed after the
     *             controler created the output directory.
     */
    @Deprecated
    ...
    // ******** --------- *******
    // The following methods are the outermost interface of the Controler. They
    // are used
    // to register infrastructure provided by components, which may or may not
    // be used
    // then, depending on what is in the Config file.
    // This is the point at which a component loader would operate - it would
    // create this thing, go through the components to see what they provide,
    // and put them
    // here.
    // These methods in principle be factored out to a Controller or
    // OuterController or
    // something, which then creates and configures a Controler based on the
    // config,
     ...
     */
    public final void addMobsimFactory(final String mobsimName,
	    final MobsimFactory mobsimFactory) {
	this.mobsimFactoryRegister.register(mobsimName, mobsimFactory);
    }

    public final void addSnapshotWriterFactory(final String snapshotWriterName,
	    final SnapshotWriterFactory snapshotWriterFactory) {
	this.snapshotWriterRegister.register(snapshotWriterName,
		snapshotWriterFactory);
    }

    public final void addPlanStrategyFactory(
	    final String planStrategyFactoryName,
	    final PlanStrategyFactory planStrategyFactory) {
	this.addOverridingModule(new AbstractModule() {
	    @Override
	    public void install() {
		addPlanStrategyBindingToFactory(planStrategyFactoryName,
			planStrategyFactory);
	    }
	});
    }

    public final void addPlanSelectorFactory(
	    final String planSelectorFactoryName,
	    final PlanSelectorFactory<Plan, Person> planSelectorFactory) {
	this.addOverridingModule(new AbstractModule() {
	    @Override
	    public void install() {
		addPlanSelectorFactory(planSelectorFactoryName,
			planSelectorFactory);
	    }
	});
    }
     ...
     * @deprecated -- try to use controler.addPlanStrategyFactory or
     *             controler.addPlanSelectoryFactory. There are cases when this
     *             does not work, which is in particular necessary if you need
     *             to re-configure the StrategyManager during the iterations,
     *             <i>and</i> you cannot do this before the iterations start. In
     *             such cases, using this method may be ok. kai/mzilske, aug'14
     */
    @Deprecated
    // see javadoc above
