...
Poniżej przedstawiono zakres prac wykonanych w~celu zgłębienia tematu problematyki optymalizacji ruchu drogowego. Zaprezentowane zostały kolejne etapy tworzenia pracy jak i~przedstawiono krótko stworzone rozwiązanie. 

\subsubsection{Studia literaturowe}
Badania rozpoczęto od poszukiwania źródeł traktujących o~opisywanym problemie. Paradoks Braessa został sformułowany w~1970 roku i~był od tego czasu wykorzystywany przy planowaniu przestrzeni i~infrastruktury wielu miast ~\cite{urban}. Udowodniono również jego występowanie w innych dziedzinach życia. Przykładem może być eksperyment przeprowadzony przez Max Planck Institute for Dynamics and Self-Organization. Poprzez symulację, udowodniono że zjawisko to występuje w sieciach elektrycznych, w których produkcja energii jest zdecentralizowana \cite{elektrony}. Badania międzynarodowego zespołu naukowców z Institut Néel oraz IEMN\footnote{Centre national de la recherche scientifique, Francja}, INP\footnote{Institut polytechnique de Grenoble, Francja} i UCL\footnote{Université catholique de Louvain, Belgia} w 2012 roku dowiodły występowanie paradoksu w mezoskopowych systemach elektronowych. W szczególności wykazano, że dodanie ścieżki elektronów w nanoskopowej sieci może paradoksalnie zmniejszyć jego przewodność. Wykazano to zarówno podczas teoretycznych symulacji i prakycznych eksperymentów w niskiej temperaturze \cite{nanoskopia}.
...
Ponieważ nie zostały znalezione żadne przesłanki wskazujące na jednoznaczną ocenę co do słuszności zamknięcia danej ulicy, w~pracy zdecydowano się na losowe przeszukiwanie przestrzeni rozwiązań.~Jednym z~rozwiązań w~przypadku takich poszukiwań są algorytmy genetyczne, które zostały wykorzystane w~pracy.
...
Paradoks Braessa nie jest jedynym twierdzeniem traktującym o~problemach komunikacyjnych miast. Wiele teorii jest opartych głównie na socjologicznych lub psychologicznych założeniach. Są to na przykład paradoks Downsa Thomsona~\cite{urban} lub prawo Lewisa Mogridge’a~\cite{lewis}. Niestety są one niemniej ważne. Biorąc pod uwagę złożoność problemu, wynik otrzymany podczas eksperymentu nie może być dowodem ani decydującym głosem w~decyzjach dotyczących ustalania rzeczywistej sieci drogowej miasta. 

\subsubsection{Opis rozwiązania GRoNO}
...
Rozdział~\ref{rozdz.optymalizacja} zawiera opis teoretyczny zagadnień związanych z~realizacją projektu GRoNO.~Wyjaśnia on wykorzystywane struktury oraz matematyczne prawa zastosowane podczas ich modyfikacji. Wskazano sposób reprezentacji struktur sieci drogowych i~ich optymalizacja.

W~rozdziale~\ref{rozdz.technologie} opisano technologie, które zostały użyte podczas tworzenia rozwiązania GRoNO.~Zawiera on opis zewnętrznych bibliotek, użytych w~pracy oraz wyjaśnienie działania kolejnych elementów systemu. Opisano również sposób wdrożenia rozwiązania.

Głównym rozdziałem pracy jest rozdział~\ref{rozdz.opis},
w którym przedstawiono aspekty teoretyczne i~praktyczne wykonanego rozwiązania. Omówiono również parametry uruchomienia projektu GRoNO wykorzystane podczas badań wraz z~ich wartościami. Na zakończenie zaprezentowano wyniki optymalizacji przykładowej sieci. Rezultaty zawierają krótkie omówienie i~analizę.
...
Poniższy rozdział zawiera opis teoretyczny zagadnień matematycznych wykorzystanych podczas realizacji projektu GRoNO.~Przybliżono w~nim użyte struktury oraz prawa wykorzystane podczas ich modyfikacji. Wskazany zostaje sposób reprezentacji struktur sieci drogowych i~ich optymalizacja.
...
Paradoks w~oryginalnym artykule wytłumaczony jest na prostym przykładzie, który został zaprezentowany poniżej. Za punkt wyjściowy przyjmujemy pewny układ dróg o~znanych czasach przejazdów.
...
Najłatwiej wyobrazić sobie powyższe kroki analizując je na schemacie przedstawionym na rysunku~\ref{fig:ogolny_schemat_alg_genetycznego}. Następnie omówiono wszystkie kroki algorytmu.
...
\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.50\textwidth]{img/selekcja_roul}
...
\begin{definition}
Wierzchołek nie połączony krawędzią z żadnym innym wierzchołkiem grafu nazywamy \textbf{wierzchołkiem izolowanym} (\textbf{ang. isolated vertex}).
\end{definition}

\begin{definition}
Dane dwa wierzchołki mogą być połączone ze sobą za pomocą więcej niż jednej krawędzi, które nazywamy \textbf{krawędzią wielokrotną} (\textbf{ang. multi-edge}).
\end{definition}

\begin{definition}
Wierzchołek może łączyć się krawędzią z samym sobą. Otrzymujemy wtedy tzw. \textbf{pętlę} (\textbf{ang. loop}). 
\end{definition}

\begin{definition}
Graf zawierający pętle lub krawędzie wielokrotne nazywamy \textbf{multigrafem} (\textbf{ang. multigraph}).
\end{definition}

\begin{definition}
Graf nie posiadający pętli oraz krawędzi podwójnych nazywamy \textbf{grafem prostym} (\textbf{ang. simple graph} lub \textbf{strict graph}).
\end{definition}

\begin{definition}
\textbf{Stopniem wierzchołka} (\textbf{ang. degree}) nazywamy liczbę krawędzi, które łączą się z danym wierzchołkiem. Jeśli graf posiada pętle, to liczymy je za 2.
\end{definition}
...
\begin{definition}
Mówimy, że \textbf{ścieżka jest prosta} (\textbf{ang. strait path} lub \textbf{simple path}), jeśli każdą krawędź/wierzchołek przechodzimy tylko jeden raz.
\end{definition}
...
\begin{definition}
Graf nazywamy \textbf{acyklicznym} (\textbf{ang. acyclic graph}), jeśli nie posiada żadnych cykli.
\end{definition}
...
\subsection{Sieć drogowa w~postaci grafu}
...
W powyższym rozdziale zostały przybliżone matematyczne prawa i twierdzenia użyte podczas realizacji pracy dyplomowej. Wyjaśniony został paradoks Braessa, który jest kluczowym założeniem pracy. Zaprezentowano struktury i prawa użyte w części praktycznej projektu GRoNO. Opis implementacji oraz wykorzystanych w związku z nią bibliotek znajduje się w rozdziale kolejnym.

\chapter{Technologie informatyczne użyte w~projekcie GRoNO}\label{rozdz.technologie} 

W niniejszym rozdziale opisane zostają technologie informatyczne, które zostały użyte podczas tworzenia rozwiązania GRoNO. Wyjaśniono działanie kolejnych elementów systemu oraz przedstawiono użyte zewnętrzne biblioteki. Opisany został również sposób wdrożenia rozwiązania jak i dane użyte podczas zrealizowanych badań.
...
\begin{itemize}
\item Każdy agent musi posiadać tylko jeden plan.
\item Plan nie musi być domyślnie wybrany ani nie musi posiadać obliczonego już wyniku.
\item Miejsce odbywania się aktywności może być opisane jedynie przez koordynaty.
\item Planowane aktywności muszą mieć oznaczony czas zakończenia.
\item Trasa może posiadać jedynie sposób transportu, bez obliczonej dokładnej trasy dotarcia do celu.
\end{itemize}

\subsection{Symulacja ruchu drogowego}\label{rozdz.symulacja}

Podczas podstawowej symulacji w~środowisku \textit{MATSim}, potrzeby przemieszczania się agentów są symulowane w~zadanym środowisku, na przykład sieci drogowej. Podczas typowej symulacji można wyróżnić pięć podstawowych etapów (Rys. \ref{fig:schemat_symulacji}):
...
\begin{figure}[htbp]
	...
	\caption{Diagram klas pakietu \textit{Apache Math Genetics}}
...
Ponadto biblioteka zawiera gotowe klasy związane z różnymi typami grafów, w tym grafami skierowanymi opisywanymi w podrozdziale \ref{sec:podstawy_grafy}. Działając na gotowej strukturze można w pełni korzystać z wbudowanych algorytmów do analizy i przeszukiwania grafów. Metody te były opisywane w podrozdziale \ref{sec:przeszukiwanie_grafow} oraz \ref{sec:graf_spojny}.
...
Korzystając rozwiązania opisanego powyżej, dzięki któremu projekt jest możliwy do zainstalowania na innych maszynach, do obliczeń wykorzystano zewnętrznego dostawcę mocy obliczeniowej. Przy wyborze decydującym czynnikiem była cena rozwiązania, co w~przypadku chmury\textbf{ Microsoft Azure}~\cite{azure}, pozwoliło na darmowe rozwiązanie. Do wyboru użytkownik posiada dość szeroką gamę konfiguracji, które może dostosować idealnie do swoich potrzeb. W~przypadku poniższego projektu kluczowymi aspektami była ilość niezależnych wątków obliczeniowych. Z~sekcji maszyn wirtualnych Linux, do projektu najlepiej nadawała się warstwa podstawowa, przeznaczona do wystąpień ogólnego zastosowania. Jest to ekonomiczna opcja dla obciążeń związanych z~tworzeniem aplikacji, serwerów testowych i~innych aplikacji, które nie wymagają równoważenia obciążeń, automatycznego skalowania i~maszyn wirtualnych korzystających z~dużej ilości pamięci. Na rysunku \ref{fig:azure_dostepne_opcje} przedstawiamy dostępne rozwiązania. W~pracy wykorzystywana została opcja \textit{A4}.
...
W powyższym rozdziale opisano technologie informatyczne związane z projektem GRoNO. Przybliżono działanie symulatora \textit{MATSim} oraz objaśniono jego ustawienia. Przedstawiono również badany w niniejszej pracy dyplomowej przykładowy scenariusz symulacji transportu drogowego. Zaprezentowane zostały biblioteki informatyczne, z których skorzystano w trakcie realizacji projektu. Na zakończenie opisany został sposób uruchomienia rozwiązania w chmurze. Wyniki badań uzyskanych w wyniku tego uruchomienia, wraz z analizą, znajdują się w rozdziale kolejnym.

\chapter{Opis projektu GRoNO}\label{rozdz.opis} 
...
\section{Obsługa projektu GRoNO}
...
\section{Ustawienia projektu GRoNO zastosowane podczas badań}
