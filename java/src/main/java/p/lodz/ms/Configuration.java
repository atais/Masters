...
public class Configuration {
    ...
    public Configuration readXMLFile(File configFile) throws ConfigurationException {
	return readConfig(new XMLConfiguration(configFile));
    }

    public Configuration readXMLFile(String configFile) throws ConfigurationException {
	return readConfig(new XMLConfiguration(configFile));
    }
	...
	this.scenarioIterations = config.getInteger("scenario.iterations", DEFAULTSCENARIOITER);

	this.gaPopulationSize = config.getInteger("genetics.population-size", DEFAULTGAPOPULATION);
	this.gaMaxGenerations = config.getInteger("genetics.max-generations", DEFAULTGAMAXGEN);
	this.gaElitismRate = config.getDouble("genetics.elitism-rate", DEFAULTGAELITISM);
	this.gaCrossoverRate = config.getDouble("genetics.crossover-rate", DEFAULTGACROSSOVER);
	this.gaMutationRate = config.getDouble("genetics.mutation-rate", DEFAULTGAMUTATION);
	this.gaTournamentArity = config.getInteger("genetics.tournament-arity", DEFAULTGAARITY);
	return this;
    }
